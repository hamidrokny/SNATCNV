clear;
%%% CHRname initialization
fprintf('initializing...\n')
chr_initialization;
%%%%%%%%%%%%%%%%%%%%%%%%% SETTING PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%%
CNV_type='Del';
CHRname=CHRname_19; %genome build
PATH_INPUT='../input';
PATH_OUTPUT='../output';
FILE_CASE_NAME='ASD_cnv_case.txt';
FILE_CONTROL_NAME='ASD_control_case.txt';
NUMBER_OF_CHR=24;
CNV_trd=0; % ignore those base paires with less than cnv_trd overlapped cnv
%%%%%%%%%%%%%%%%%%%%%%%%% SETTING PARAMETERS %%%%%%%%%%%%%%%%%%%%%%%%%%
...
fname=strcat(PATH_OUTPUT,'/','significant_regions_based_on_cnv_',CNV_type);
...
for chr_id=22 : NUMBER_OF_CHR
    ...
    number_case= nnz(indx); %cnv based
    indx=strcmp(CONTROL_cnv{1,6}, CNV_type);
    number_control=nnz(indx); %cnv based

    CNVarray_case=0;
    CNVarray_control=0;
    maskarray=0;
    CNVarray_case(cell2mat(CHRname(chr_id,3)),1)=0;
    CNVarray_control(cell2mat(CHRname(chr_id,3)),1)=0;
    maskarray(cell2mat(CHRname(chr_id,3)),1)=0;

    %%% filter on case chr
    indx=strcmp(CASE_cnv{1,1}, CHRname(chr_id));  
    temp = cellfun(@(x) x(indx), CASE_cnv, 'UniformOutput', false);
    %%% filter on case cnv type
    ...
    indx=strcmp(CONTROL_cnv{1,1}, CHRname(chr_id));
    temp = cellfun(@(x) x(indx), CONTROL_cnv, 'UniformOutput', false);   
    %%% filter on control cnv type
    indx=strcmp(temp{1,6}, CNV_type);
    number_chr_type=nnz(indx);
    SCH_chr_type = cellfun(@(x) x(indx), temp, 'UniformOutput', false);  
    %%% count number of overlapping CNVs
    ...
    %%% for the last region
    region_start=diff_row(i,1)+1;
    region_end=cell2mat(CHRname(chr_id,3)); %end position in the chr
